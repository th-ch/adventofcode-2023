...
fn predict(numbers: []const i64) !i64 {
    ...
    for (1..n) |i| {
        ...
        var delta = number - numbers[i - 1];
        var level: usize = 0;
        var j = i;
        while (j > 0) {
            derivatives[level][j - 1] = delta;
            if (j > 1) {
                delta = derivatives[level][j - 1] - derivatives[level][j - 2];
            }
            j -= 1;
            level += 1;
        }
    ...
    for (0..n - 1) |i| {
        const level = n - 1 - i;
        derivatives[level - 1][i + 1] = derivatives[level][i] + derivatives[level - 1][i];
    }

    return derivatives[0][n - 1] + numbers[n - 1];
}
