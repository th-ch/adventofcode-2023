...
std::deque<int> ParseLine(const std::string& line) {
  ...
  do {
    ...
    numbers.push_front(atoi(line.substr(pos, next_pos - pos).c_str()));
    pos = next_pos + 1;
  } while (pos > 0);
  return numbers;
}
...
int FindNext(const std::deque<int>& numbers) {
  ...
  while (true) {
    ...
    if (diffs.size() >= 3 && diffs.at(diffs.size() - 3).front() == 0 &&
        diffs.at(diffs.size() - 3).at(1) == 0 &&
        diffs.at(diffs.size() - 3).at(2) == 0 &&
        diffs.at(diffs.size() - 3).back() == 0) {
      diffs.pop_back();
      diffs.pop_back();
      diffs.pop_back();
      break;
