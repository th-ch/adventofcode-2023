...
const expansion_factor = if (builtin.is_test) 10 else 1_000_000;
...
fn run(input: [:0]const u8) !i64 {
    ...
    for (input) |c| {
        if (c == '\n') {
            ...
            expanded_y += 1;
            if (!galaxy_row_lookup.items[y]) {
                expanded_y += expansion_factor - 1;
            }
            continue;
        }
        if (!galaxy_column_lookup.items[x]) {
            expanded_x += expansion_factor - 1;
        }
...
test {
    ...
    try std.testing.expectEqual(@as(i64, 1030), result);
}
