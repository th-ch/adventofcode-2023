...
fn parseGroups(str: []const u8) ![]u8 {
    ...
    const initial_count = list.items.len;
    for (0..4) |_| {
...
fn produceConditionString(original: []const u8) ![]const u8 {
...
fn runLine(line: []const u8) !i64 {
    ...
    const condition_string = try produceConditionString(it.next().?);
...
test {
    ...
    try std.testing.expectEqual(@as(i64, 525152), result);
}
