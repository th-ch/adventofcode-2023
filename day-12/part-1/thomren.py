...
class ThomrenSubmission(SubmissionPy):
    def run(self, s: str):
        ...
        for line in s.splitlines():
            ...
            contiguous = tuple(int(x) for x in contiguous.split(","))
...
def test_thomren():
    ...
    """
    assert combinations("???.###", (1, 1, 3)) == 1
    assert combinations(".??..??...?##.", (1, 1, 3)) == 4
    assert combinations("?#?#?#?#?#?#?#?", (1, 3, 1, 6)) == 1
    assert combinations("????.#...#...", (4, 1, 1)) == 1
    assert combinations("????.######..#####.", (1, 6, 5)) == 4
    assert combinations("?###????????", (3, 2, 1)) == 10
    assert (
