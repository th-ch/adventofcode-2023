...
impl<'a> Contraption<'a> {
    ...
    fn start_beam(&mut self, start: (usize, usize, Direction)) -> usize {
        let mut energized = 0;
...
fn run(input: &str) -> usize {
    ...
    for x in 0..contraption.width {
        max = max.max(contraption.start_beam((x, 0, Down)));
        contraption.reset_beams();
        max = max.max(contraption.start_beam((x, contraption.height - 1, Up)));
        contraption.reset_beams();
    }
    for y in 0..contraption.height {
        max = max.max(contraption.start_beam((0, y, Right)));
        contraption.reset_beams();
        max = max.max(contraption.start_beam((contraption.width - 1, y, Left)));
        contraption.reset_beams();
    }
    max
}
