...
fn getNeighbors(vertex: Vertex, grid: Grid) [3]?Vertex {
    ...
    switch (vertex.direction) {
        .up => {
            ...
            if (vertex.position.x < grid.width - 1) {
                result[1] = Vertex{
                    .position = .{ .x = vertex.position.x + 1, .y = vertex.position.y },
                    .direction = .right,
                    .count = 0,
            ...
            if (vertex.count < 2 and vertex.position.y > 0) {
                result[2] = Vertex{
        ...
        .down => {
            ...
            if (vertex.count < 2 and vertex.position.y < grid.height - 1) {
                result[2] = Vertex{
                    .position = .{ .x = vertex.position.x, .y = vertex.position.y + 1 },
                    .direction = .down,
                    .count = vertex.count + 1,
        ...
        .left => {
            ...
            if (vertex.position.y < grid.height - 1) {
                result[1] = Vertex{
                    .position = .{ .x = vertex.position.x, .y = vertex.position.y + 1 },
                    .direction = .down,
                    .count = 0,
            ...
            if (vertex.count < 2 and vertex.position.x > 0) {
                result[2] = Vertex{
                    .position = .{ .x = vertex.position.x - 1, .y = vertex.position.y },
                    .direction = .left,
                    .count = vertex.count + 1,
        ...
        .right => {
            ...
            if (vertex.count < 2 and vertex.position.x < grid.width - 1) {
                result[2] = Vertex{
                    .position = .{ .x = vertex.position.x + 1, .y = vertex.position.y },
                    .direction = .right,
                    .count = vertex.count + 1,
...
fn run(input: [:0]const u8) !i64 {
    ...
    while (it.next()) |entry| {
        if (entry.key_ptr.position.x == grid.width - 1 and entry.key_ptr.position.y == grid.height - 1) {
